...
pub struct NewEntry {
    pub directory: bool,
    pub metadata: model::ConceptMetadata,
...
fn read_body(reader: impl Read) -> Result<String> {
    ...
    reader
        .take(model::MAX_FILE as u64 + 1)
        .read_to_end(&mut bytes)?;
    ensure!(
        bytes.len() <= model::MAX_FILE,
...
impl Workspace {
    ...
    pub fn new_entry(&self, name: &str, path: &str, options: NewEntry) -> Result<()> {
        ...
        let original = self.knowledge()?;
        let mut content = model::subtree(&original, name);
        ...
        while let Some(directory) = parent {
            ...
            if content
                .get(&index)
                .is_some_and(|bytes| !model::is_generated_index(bytes))
            {
        ...
        model::root_index(&mut tree, &names)?;
            ...
            tree.values().all(|bytes| bytes.len() <= model::MAX_FILE),
            "generated file exceeds 16 MiB"
        );
        ensure!(
            tree.values().map(Vec::len).sum::<usize>() <= model::MAX_TREE,
